...
module.exports.onLoad = function () {
    ...
    const path = resolve(__dirname, "noprefix", "shortcutdata.json");
    const pathGif = resolve(__dirname, "noprefix", "shortcutGif");
...
module.exports.handleEvent = async function ({ event, api, Users }) {
    ...
    if (data.some(item => item.input == body)) {
        ...
        const path = resolve(__dirname, "noprefix", "shortcutGif", `${dataThread.id}.gif`);
    ...
    switch (handleReply.type) {
        ...
        case "requireGif": {
            ...
            if (body.match(/(http(s?):)([/|.|\w|\s|-])*\.(?:gif|GIF)/g)) {
                const pathGif = resolve(__dirname, "noprefix", "shortcutGif", `${id}.gif`);
                try {
        ...
        case "-a": {
            ...
            if (data.length == 0) return api.sendMessage("「Shortcut」hiện tại nhóm của bạn chưa có shortcut nào được set!", threadID, messageID);
                ...
                for (const single of data) {
                    const path = resolve(__dirname, "noprefix", "shortcutGif", `${single.id}.gif`);
                    var existPath = false;
